...
volatile uint32_t *ctrl_chain = (uint32_t*)0xf0030000;
                    ...
                    unsigned char *data_out)
{
	...
	volatile uint32_t *data = &ctrl_chain[DATA_BIT]; 

  unsigned char cmd_gen_chain = 3;
   
	  // send data_in to hw core
	for (i = 0; i < 8; i++)
    data[i] = ((uint32_t*)pub_seed)[i]; 
	
	  // send pub_seed to hw core
	for (i = 0; i < 8; i++)
		data[i+8] = ((uint32_t*)data_in)[i]; 
  ...
  // send command code
  ctrl_chain[CMD_BIT] = (cmd_gen_chain << 8);

  // trigger start for gen_chain_with_sha module
  ctrl_chain[CONTROL_BIT] = ((start << 8) | ((start+steps-1) << 16) | START);
  
	// hw core running/busy
	while (ctrl_chain[BUSY_BIT] & BUSY == 1);
	
	// return output from hw core
	  // return data_out
	for (i = 0; i < 8; i++)
	  ((uint32_t*)data_out)[i] = ctrl_chain[i+4];
